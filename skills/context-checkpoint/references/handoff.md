...
13. 在 handoff 条目中记录已复制和已丢弃产物, 方便用户按需要求后续修正.
14. 创建或更新目标会话文件夹.
...
19. 校验已复制文件和已改写引用.
20. 报告源文件夹, 目标文件夹, 已更新文件, 已复制产物, 已丢弃产物和引用改写, 然后输出目标会话文件夹的 status summary.
...
- 在 handoff 条目中记录已复制和已丢弃产物, 方便用户审阅并要求后续修正.
...
然后输出目标会话文件夹的 status summary. status summary 格式见 `status.md`.
